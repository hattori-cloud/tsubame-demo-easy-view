...
assert(html.includes("function setAnalysisOperationScopeFromCenter(mode='modal',targetPage='')"),'scope setup helper missing');
...
assert(html.includes("if(kind==='work'){setAnalysisOperationScopeFromCenter('work','work');go('work');return}"),'work scoped route missing');
...
assert(html.includes("if(ANALYSIS_OPERATION_SCOPE?.mode==='work'&&ANALYSIS_OPERATION_SCOPE.targetPage&&id!==ANALYSIS_OPERATION_SCOPE.targetPage)ANALYSIS_OPERATION_SCOPE=null"),'page scope clear on navigation missing');
