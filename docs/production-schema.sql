...
create table documents (
  ...
  retention_reviewed_by_user_id uuid references users(id),
  retention_review_note text,
  original_filename text,
  content_type text,
  size_bytes bigint check (size_bytes is null or size_bytes >= 0),
  storage_key text,
  storage_version_id text,
  storage_state text not null default 'not_uploaded'
    check (storage_state in ('not_uploaded','quarantine','active','blocked','restore_only','purged')),
...
create table document_purge_requests (
  ...
  state text not null default 'requested'
    check (state in ('requested','approved','rejected','executed','failed','cancelled')),
  ...
  check (
    state not in ('approved','executed','failed')
...
create trigger work_import_changes_append_only_guard
before update or delete on work_import_changes
for each row execute function reject_append_only_mutation();
...
create index work_import_batches_created_idx on work_import_batches (created_at desc, id);
create index work_import_batches_status_idx on work_import_batches (status, created_at desc);
