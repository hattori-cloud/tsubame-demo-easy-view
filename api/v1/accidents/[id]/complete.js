...
const {completeAccident}=require('../../../_lib/safety-store');
...
module.exports=async function handler(req,res){
  ...
  try{
    ...
    const record=await completeAccident({user,id:String(req.query.id||''),expectedVersion:parseIfMatchHeader(req.headers['if-match']),reason:String(req.body?.reason||''),requestId:rid});
    applySecurityHeaders(res);res.setHeader('X-Request-Id',rid);setVersionEtag(res,record.version);
    return res.status(200).json({accident:record})
