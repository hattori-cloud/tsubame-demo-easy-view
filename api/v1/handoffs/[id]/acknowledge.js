...
const {acknowledgeHandoff}=require('../../../_lib/workflow-store');
...
module.exports=async function handler(req,res){
  ...
  try{
    ...
    const handoff=await acknowledgeHandoff({user,id:String(req.query.id||''),requestId:rid});
    applySecurityHeaders(res);res.setHeader('X-Request-Id',rid);return res.status(200).json({handoff})
