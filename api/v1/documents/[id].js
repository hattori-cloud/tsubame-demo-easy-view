...
const {updateDocumentMetadata}=require('../../_lib/credential-store');
...
module.exports=async function handler(req,res){
  ...
  try{
    ...
    const document=await updateDocumentMetadata({user,identity,id:String(req.query.id||''),body:req.body||{},expectedVersion:parseIfMatchHeader(req.headers['if-match']),requestId:rid});
    applySecurityHeaders(res);res.setHeader('X-Request-Id',rid);setVersionEtag(res,document.version);
