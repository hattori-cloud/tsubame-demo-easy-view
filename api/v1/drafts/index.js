...
module.exports=async function handler(req,res){
  ...
  try{const identity=await authenticateRequest(req),user=resolveCurrentUser(identity),rid=requestId(req);const drafts=await listDrafts(user);applySecurityHeaders(res);res.setHeader('X-Request-Id',rid);return res.status(200).json({drafts})}
