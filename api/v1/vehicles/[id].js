...
const {getVehicle,updateVehicle}=require('../../_lib/vehicle-store');
...
module.exports=async function handler(req,res){
  try{
    ...
    if(req.method==='GET'){
      const vehicle=await getVehicle(user,id);applySecurityHeaders(res);res.setHeader('X-Request-Id',rid);setVersionEtag(res,vehicle.version);return res.status(200).json({vehicle})
    }
    if(req.method==='PATCH'){
