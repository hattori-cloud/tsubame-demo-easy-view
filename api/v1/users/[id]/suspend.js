...
module.exports=async function handler(req,res){
  ...
  try{
    ...
    if(!identity.mfa){const e=new Error('利用者停止にはMFA確認済みセッションが必要です');e.status=403;e.code='MFA_REQUIRED';throw e}
    const rid=requestId(req),user=await setUserState({actor,userId:String(req.query.id||''),state:'suspended',expectedVersion:parseIfMatchHeader(req.headers['if-match']),reason:String(req.body?.reason||''),requestId:rid});
