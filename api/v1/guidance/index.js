...
module.exports=async function handler(req,res){
  try{
    ...
    if(req.method==='GET'){const result=await listSupport(user,'guidance',req.query||{});applySecurityHeaders(res);res.setHeader('X-Request-Id',rid);return res.status(200).json(result)}
    if(req.method==='POST'){const record=await createSupport({user,kind:'guidance',body:req.body||{},requestId:rid});applySecurityHeaders(res);res.setHeader('X-Request-Id',rid);res.setHeader('ETag','"'+record.version+'"');return res.status(201).json({record})}
