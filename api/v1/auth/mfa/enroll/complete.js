...
const {getMfaChallenge,enrollUserMfa,markMfaVerified,recordMfaFailure,createSession,writeAuthAudit,hashMetadata}=require('../../../../_lib/auth-store');
...
module.exports=async function handler(req,res){
  ...
  if(!ok){try{await recordMfaFailure(challenge.id);await writeAuthAudit({action:'mfa_enrollment_failed',userId:challenge.user_id,result:'denied',requestId:id,summary:'TOTP enrollment verification failed'})}catch(_){};return res.status(401).json(errorBody('MFA_ENROLLMENT_FAILED','MFA登録を確認できません',id))}
  ...
  try{session=await withTransaction(async client=>{
    ...
    if(err?.code==='SESSION_NOT_ALLOWED')return res.status(401).json(errorBody('MFA_ENROLLMENT_FAILED','MFA登録を確認できません',id));
