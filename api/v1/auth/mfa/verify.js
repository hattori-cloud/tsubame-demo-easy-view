...
const {getMfaChallenge,findMfaMaterial,findCredentialAccountById,markMfaVerified,recordMfaFailure,recordLoginFailure,clearLoginFailures,createSession,writeAuthAudit,hashMetadata}=require('../../../_lib/auth-store');
...
module.exports=async function handler(req,res){
  ...
  let account;
  try{account=await findCredentialAccountById(challenge.user_id)}catch(_){return res.status(503).json(errorBody('MFA_STORE_UNAVAILABLE','追加認証保存先を利用できません',id))}
  const locked=account?.locked_until&&new Date(account.locked_until).getTime()>Date.now();
  if(!account||account.state!=='active'||account.employee_lifecycle_status==='retired'||locked)return res.status(401).json(errorBody('MFA_FAILED','追加認証を確認できません',id));
  ...
  if(!ok){
    try{
      await withTransaction(async client=>{
        ...
        await writeAuthAudit({action:'mfa_failed',userId:challenge.user_id,result:'denied',requestId:id,summary:'TOTP verification failed'},client)
      })
    }catch(_){}
