...
module.exports=async function handler(req,res){
  ...
  try{
    ...
    const result=await rollbackWorkImport({
      user,batchId:String(req.query.id||''),expectedVersion:parseIfMatchHeader(req.headers['if-match']),
      reason:req.body?.reason,requestId:rid
    });
