...
async function createUser({actor,employeeId,loginId,displayName,roleLevel='self',safetyAuthority=false,scopes=[],passwordHash,resetTokenHash,requestId}){
  ...
  return withTransaction(async client=>{
    ...
    if(!['full','scoped','self'].includes(roleLevel))throw problem(422,'INVALID_ROLE','権限区分を確認してください');
    const normalized=normalizeScopes(scopes);if(roleLevel==='scoped'&&!normalized.length)throw problem(422,'SCOPE_REQUIRED','担当管理者には担当範囲が必要です');
    const mfaRequired=roleLevel!=='self';
    ...
    return {...row,employee_no:emp.employee_no,scopes:normalized,setup_expires_in:1800}
  })
}
async function updateUserAccess({actor,userId,roleLevel,safetyAuthority,scopes,expectedVersion,requestId}){
  ...
  return withTransaction(async client=>{
    ...
    return {...after,scopes:role==='scoped'?normalized:[]}
  })
}
...
async function completePasswordReset({tokenHash,passwordHash,requestId}){
  return withTransaction(async client=>{
    ...
    await query('update password_reset_tokens set used_at=now() where id=$1',[t.id],client);
...
module.exports={listUsers,createUser,updateUserAccess,setUserState,issuePasswordReset,completePasswordReset,normalizeScopes,assertFullAdmin};
